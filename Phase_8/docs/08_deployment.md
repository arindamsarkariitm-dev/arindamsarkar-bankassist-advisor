...
## How LangSmith tracing is safely enabled (off by default)

`capstone_build_plan.md` §0.3 flagged the risk at the very start of the project: LangSmith's *default* tracing ships full conversation payloads to a cloud service, which directly conflicts with "must not store PII in logs" (the scenario's core safety requirement). §0.3's own instruction was explicit about the fix, not just the risk: *"if you use it, gate it behind an env flag and trace only redacted text."* `src/langsmith_tracing.py` is that gate.

**Off by default.** Tracing only activates if `ENABLE_SAFE_LANGSMITH_TRACING=true` is set — absent that, `tracing_callbacks()` returns `[]` and zero data of any kind reaches LangSmith. This project's own structured logger (`src/nodes/log.py` + `src/observability.py`) remains the primary, always-on observability mechanism regardless of this flag.

**When enabled, every payload is redacted before it leaves the process**, using LangSmith's own documented mechanism for this — `Client(hide_inputs=fn, hide_outputs=fn, hide_metadata=fn)`, where each function receives the run's already-serialized inputs/outputs/metadata and returns a modified copy, applied *before* the run is sent to LangSmith's API. `src/langsmith_tracing.py` passes `src/redaction.py`'s existing `redact_text()` — the same PII layer already used by logging and memory — through a small recursive walker (`_redact_value`) so it applies to whatever shape a run payload takes (nested message lists, tool-call args, metadata dicts), not just flat strings. Verified in `tests/test_no_pii_in_langsmith.py` against a realistic nested payload: known PII (amounts, phone numbers, account numbers, emails, customer names) is stripped, while safe reference IDs (`TXN-`, `ACC-`, etc.) and non-text fields (confidence scores, tool names) pass through untouched.

**A real gap found and deliberately avoided, not a hypothetical concern**: the standard `LANGCHAIN_TRACING_V2` / `LANGSMITH_TRACING` env-var names were deliberately *not* used for the enable flag. Reading the installed `langsmith` SDK (`langsmith/utils.py`'s `tracing_is_enabled()`, checked via `langchain_core`'s tracing context) confirmed that setting either of those names — in either the `LANGCHAIN_` or `LANGSMITH_` namespace — switches on LangChain's own **global auto-tracer**, which uses a plain, unredacted `Client()` and fires independently of any explicit callback a call site passes. Had the flag been named `LANGCHAIN_TRACING_V2`, enabling it would have started *two* tracers side by side: the redacting one built here, and LangChain's own raw one — silently defeating the entire point. `ENABLE_SAFE_LANGSMITH_TRACING` was chosen specifically because it shares no namespace prefix with either reserved name, so it cannot trigger that path.

**Where it's wired in**: every LLM call in the system already funnels through exactly two functions in `src/resilience.py` (`invoke_with_retry`, `invoke_structured_with_resilience`) — confirmed by grepping the whole `src/` tree for `.invoke(`, which found no other call site. Both now pass `config={"callbacks": tracing_callbacks()}`, so wiring tracing in required touching one file, not each of the six places `ChatOpenAI` is constructed.

**To enable it**: set `ENABLE_SAFE_LANGSMITH_TRACING=true`, `LANGCHAIN_API_KEY=<your LangSmith key>`, and `LANGCHAIN_PROJECT=bankassist-advisor` in `.env` (locally) or Streamlit's Secrets panel (cloud) — see `.env.example`.
