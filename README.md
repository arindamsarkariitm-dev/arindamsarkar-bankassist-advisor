...
tests/          the golden test set (eval_set.yaml) + the pytest-style
                safety suite (test_no_pii_in_logs.py, test_refusals.py,
                test_grounding.py, test_escalation.py, test_tool_guards.py,
                test_no_pii_in_langsmith.py)
logs/           redacted JSONL trace log the running system writes to
docs/           cross-cutting deliverables not tied to one phase
                (demo_script.md)
evidence/       cross-cutting evidence not tied to one phase
                (the demo script's live-app screenshots)
...
See `Phase_8/docs/08_deployment.md` for the full assumptions/limitations writeup, and `Phase_9/docs/09_evaluation_report.md` for the measured evaluation results and root-cause analysis.
